/*jslint browser: true, bitwise: true, plusplus: true, sloppy: true, vars: true, white: true, undef: true, white: true */
/*global define */
...
define(function () {
	...
	Bn.prototype = {
		...
		/**
		 *	(x+n) where x is a bigInt and n is an integer.
		 *
		 *	@param integer n
		 *	@returns Bn result
		 */
		addInt: function (n) {
			return new Bn(addInt(this.bigint, n));
		},
		 ...
		 */
		equals: function (y) {
			return equals(this.bigint, y.bigint);
		},

		/**
		 *	is bigint x equal to integer n?
		 *
		 *	@param integer n
		 *	@returns boolean result
		 */
		equalsInt: function (n) {
			return equalsInt(this.bigint, n);
		},

		/**
		 *	greatest common divisor of bigInts x and y (each with same number of elements).
		 *
		 *	@param Bn y
		 *	@returns Bn result
		 */
		gcd: function (y) {
			return new Bn(GCD(this.bigint, y.bigint));
		},
		 ...
		 */
		greater: function (y) {
			return greater(this.bigint, y.bigint);
		},
		 ...
		 *	@returns boolean result
		 */
		greaterShift: function (y, shift) {
			return greaterShift(this.bigint, y.bigint, shift);
		},

		/**
		 *	(x**(-1) mod n) for bigInts x and n.  If no inverse exists, it returns null
		 *
		 *	@param Bn n
		 *	@returns Bn result
		 */
		inverseMod: function (n) {
			var result = inverseMod(this.bigint, n.bigint);
		...
		/**
		 *	x**(-1) mod n, for integers x and n.  Return 0 if there is no inverse
		 *
		 *	@param integer n
		 *	@returns integer result
		 */
		inverseModInt: function (n) {
			return inverseModInt(this.bigint, n);
		},
		 ...
		 *	@returns boolean result
		 */
		millerRabin: function (b) {
			return new Bn(millerRabin(this.bigint, b.bigint));
		},
		 ...
		 *	@returns boolean result
		 */
		millerRabinInt: function (b) {
			return new Bn(millerRabinInt(this.bigint, b));
		},
		 ...
		 */
		modInt: function (n) {
			return modInt(this.bigint, n);
		},

		/**
		 *	like BN_mod, but returns non-negative remainder
 		 *	(i.e.,  0 <= r < |y|  always holds)
		 *
		 *	@param Bn y
		 *	@returns Bn result
		 */
		nnmod: function (y) {
			...
			// now   -|y| < r < 0,  so we have to set  r := r + |y|
			if (y.isNegative()) {
				return res.sub(y);
			} else {
				return res.add(y);
		...
		/**
		 *	x*y for bigInts x and y. This is faster when y<x.
		 *
		 *	@param Bn y
		 *	@returns Bn result
		 */
		mul: function (y) {
			return new Bn(mult(this.bigint, y.bigint));
		},
		...
		toString: function (base) {
			return bigInt2str(this.bigint, base || 16).toLowerCase();
		}
	};
	 ...
	 *	@returns Bn result
	 */
	Bn.randTruePrime = function (k) {
		return new Bn(randTruePrime(k));
	};
	 ...
	 *	@returns Bn result
	 */
	Bn.randProbPrime = function (k) {
		return new Bn(randProbPrime(k));
	};

	/**
	 *	Créer un bignum qui vaut 0.
	 *
	 *	@returns Bn result
	 */
	Bn.Zero = function () {
